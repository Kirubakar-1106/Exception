...
import com.onesofts.employee.exceptions.GenderNotFoundException;
import com.onesofts.employee.exceptions.NameNotFoundException;
import com.onesofts.employee.exceptions.SalaryNotFoundException;
...
public class EmployeeService {
	...
	public List<Employee> getbysalary(int a, int b) {
		List<Employee>temp=empdao.getbysalary(a, b);
		if(temp.isEmpty()) {
			throw new SalaryNotFoundException("there is no data found for given salary");
		}
		else {
		return temp;
	}
	...
	public List<Employee> getbygender(String gender) {
		List<Employee>temp= empdao.getbygender(gender);
		if(temp.isEmpty()) {
			throw new GenderNotFoundException("there is no data found for "+gender);
		}
		else {
		return temp;
		}
